...
def get_medical_response(message):
    ...
    if any(word in message for word in ['emergency', 'urgent', 'severe', 'critical', 'help me']):
    ...
    try:
        ...
        if detect_disease_query(message):
            ...
            if ai_response:
                return f"🤖 **AI Prevention Tips:**\n\n{ai_response}"
        
        elif 'vaccination' in message or 'vaccine' in message:
            # Handle vaccination with AI enhancement
            ai_response = llama_service.get_medical_response(f"Provide information about vaccination: {message}")
            if ai_response:
                return f"🤖 **AI Vaccination Information:**\n\n{ai_response}"
...
def detect_symptom_query(message):
    ...
    symptom_indicators = ['i have', 'experiencing', 'symptoms', 'feeling', 'pain', 'ache', 'hurt', 'sick']
    common_symptoms = ['headache', 'fever', 'cough', 'cold', 'nausea', 'vomiting', 'diarrhea', 'fatigue', 'tired']
    
    has_symptom_indicator = any(indicator in message for indicator in symptom_indicators)
...
def detect_prevention_query(message):
    ...
    prevention_indicators = ['prevent', 'prevention', 'avoid', 'protect', 'stay healthy', 'tips', 'how to']
...
def extract_disease_names(message):
    ...
    all_diseases = list(MEDICAL_KB.keys()) + ['cancer', 'covid', 'malaria', 'tuberculosis', 'asthma', 'migraine', 'pneumonia', 'bronchitis']
